...
simfin = simfin.sort_values(by=['tic', 'date'],ignore_index=True)
# get unique dates
dates = simfin.date.unique()

tickers = simfin.tic.unique().tolist()
...
for tic in tickers:
    ...
    df = pd.merge(df, simfin.loc[simfin.tic==tic], on=['tic','date'], how='left')
...
final.to_csv('all_us_stocks.csv')
